...
if __name__ == '__main__':
    ...
    houses = os.listdir(House_dir)
    available_houses = []
    for house in houses:
        if os.path.isfile(os.path.join(House_dir, house, 'house.obj')):
            available_houses.append(house)
            if len(available_houses) == 500:
                break
    ruined = []
    for house in available_houses:
        ...
        connectedCoor = house_obj.connectedCoors
        cam = task.env.cam
        for coor in connectedCoor:
            gx, gy = coor
            if house_obj.connMap[gx, gy] != 0 or gx % 10 != 0 or gy % 10 != 0:
                continue
            coor_dir = os.path.join(dataset_root, house, 'gx_{}_gy_{}'.format(gx, gy))
            if not os.path.isdir(coor_dir):
                os.mkdir(coor_dir)
            print(coor)
            x, y = house_obj.to_coor(gx, gy, True)
            info = {'pos': [x, y], 'yaw': 0}
            task.set_state(info)
            top_map, _ = task.env.gen_localmap(scale=3)
            top_map = top_map[:, :, ::-1]
            b, g, r = top_map[200, 200]
            if b < 80 and b > 30 and g > 30 and g < 80 and r > 240:
                cv2.imwrite(os.path.join(coor_dir, 'topdown_view.png'), top_map)
                for yaw in [0, 45]:
                    info = {'pos': [x, y], 'yaw': yaw}
                    task.set_state(info)
                    for i, mod in enumerate(['rgb', 'sem', 'ins', 'depth']):
                        cube_map = task.env.render_cube_map(modes[i])
                        cube_map = cube_map[:, :4 * cube_map.shape[1] // 6]
                        if mod == 'depth':
                            infmask = cube_map[:, :, 1]
                            cube_map = cube_map[:, :, 0] * (infmask == 0)
                        else:
                            cube_map = cube_map[:, :, ::-1]
                        cv2.imwrite(os.path.join(coor_dir, 'mode={}_{}.png'.format(mod, yaw)), cube_map)
                info = {'pos': [x, y, 3.6], 'yaw': 0}
                task.set_state(info)
                cube_map = task.env.render_cube_map(modes[1])
